...
//! - **GetStatus**: Query ongoing scan status (progress, files, duplicates)
//! - **CancelScan**: Stop an active scan gracefully
//! - **GetResults**: Retrieve duplicate groups and remediation recommendations
//! - **RemediateDuplicates**: Execute deduplication (hardlink, clone, delete)
//!
//! ## Note
//! The proto stubs are generated by `build.rs` into `src/proto/`.
//! On first build, run `cargo build` to generate them.
//!
//! ## Port Binding
//! Default: TCP 50051 (configurable in config.toml)
//! All nodes listen on 0.0.0.0:50051 for incoming peer connections.
//!
//! ## Security
//! - **Current**: No authentication (full admin required anyway)
//! - **Future**: mTLS + JWT bearer tokens for cluster auth
